...
def return_holes(points, max_circumradius=0.4, max_ratio_radius_area=0.2, height_clustering=False, eps=0.1):
    """
    Returns polygons representing holes.
    ...
    if len(big_triangles) != 0:
        poly_holes = triangles_to_holes(points, tri.simplices, big_triangles,
                                   height_clustering, eps=eps)

        poly_holes = [poly_holes] if type(poly_holes) == Polygon else list(poly_holes)
    else:
        poly_holes = Polygon()
